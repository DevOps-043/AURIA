      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#0a0a0f]/95 backdrop-blur-xl shadow-2xl shadow-black/50">
        ...
        <div className="p-4">
              ...
              <div>
                <p className="text-xs font-bold text-white">
                  {phase === 'ready' ? 'Lista para instalar' :
                   ...
                   phase === 'error' ? 'Error de actualizacion' :
                   'Nueva version disponible'}
                </p>
                <p className="text-[10px] text-gray-500 font-medium mt-0.5">
                  {phase === 'error' ? error : `AQELOR v${version}`}
                </p>
              </div>
            </div>
          ...
          {phase === 'downloading' && (
            <div className="mb-3">
              ...
              <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#22D3EE] rounded-full transition-all duration-500 ease-out shadow-[0_0_8px_rgba(34,211,238,0.4)]"
                  style={{ width: `${progress}%` }}
                />
              </div>
            </div>
          ...
          {releaseNotes && phase === 'available' && (
            ...
            >
              <p className="text-[10px] text-[#22D3EE] font-bold uppercase tracking-widest flex items-center gap-1">
                Ver novedades
            ...
            {phase === 'error' && (
              <button
                onClick={() => {
                  setError(null)
                  setPhase('hidden')
                  window.auria?.updater.checkForUpdates()
                }}
